...
class ClassDefinitionNode(ASTNode, Usable):
    def __init__(
        self,
        class_name: str,
    ...
    def is_validated(self) -> bool:
        return all((
            ...
            self.superclass.is_validated() if self.superclass else True,
            all((x.is_validated() for x in self.generic_params)) if self.generic_params else True,
            all((f.is_validated() for f in self.fields_definitions)),
            all((f.is_validated() for f in self.static_fields_definitions)),
            all((m.is_validated() for m in self.methods_definitions)),
            all((m.is_validated() for m in self.static_methods_definitions)),
        ))
    ...
    def is_valid(self) -> bool:
        return all((
            ...
            all((f.is_valid() for f in self.static_fields_definitions)),
            all((m.is_valid() for m in self.methods_definitions)),
            all((m.is_valid() for m in self.static_methods_definitions)),
        ))
